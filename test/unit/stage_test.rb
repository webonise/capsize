...
class StageTest < ActiveSupport::TestCase
  ...
  def test_deployment_possible_vars
    ...
    assert_not_nil stage.effective_configuration(:repo_url)
    ...
    # remove a config
    stage.configuration_parameters.find_by_name('repo_url').destroy rescue nil
    project.configuration_parameters.find_by_name('repo_url').destroy rescue nil

    stage = Stage.find(stage.id) # stage.reload would not clear attr_accessor

    assert_nil stage.effective_configuration(:repo_url)
    assert_not_nil stage.effective_configuration(:application)

    assert !stage.deployment_possible?
    assert_not_nil stage.deployment_problems[:repo_url]
    assert_nil stage.deployment_problems[:application]

    # add it again
    config = stage.configuration_parameters.build(:name => 'repo_url', :value => 'svn://bla.com/trunk')
    config.save!
    stage = Stage.find(stage.id) # stage.reload would not clear attr_accessor
    ...
    assert_nil stage.deployment_problems[:repo_url]
